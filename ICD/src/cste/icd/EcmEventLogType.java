...
public enum EcmEventLogType {
	...
	EcmEventLogType(byte type,String name){
		this.eventType = type;
		this.name = name;
	}
	
	public static EcmEventLogType fromValue(byte type){
		for(EcmEventLogType d : EcmEventLogType.values()){
			 if ( d.getBytes() == type)
				 return d;
		}
		return INVALID;
